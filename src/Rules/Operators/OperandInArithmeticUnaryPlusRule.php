 ...
 */
class Operand_In_Arithmetic_Unary_Plus_Rule implements Rule
{
    ...
    public function process_node(Node $node, Scope $scope): array
    {
        ...
        if (!$this->helper->is_valid_for_arithmetic_operation($scope, $node->expr)) {
            ...
            $messages[] = Rule_Error_Builder::message(sprintf('Only numeric types are allowed in unary +, %s given.', $var_type->describe(Verbosity_Level::type_only())))->identifier('unaryPlus.nonNumeric')->build();
        }
        return $messages;
    }
}